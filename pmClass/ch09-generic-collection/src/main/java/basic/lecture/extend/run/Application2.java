...
public class Application2 {
    public static void main(String[] args) {
        ...
        * */
        ...
        // 토끼 농장 자체가 생성 불가능 한것은 매개변수로 사용 불가능
        // wildCardFarm.anyType(new RabbitFarm<Mammal>()); 불가능
        wildCardFarm.anyType(new RabbitFarm<Rabbit>(new Rabbit()));
        wildCardFarm.anyType(new RabbitFarm<Bunny>(new Bunny()));
        wildCardFarm.anyType(new RabbitFarm<DrunkenBunny>(new DrunkenBunny()));

        // 와일드카드의 매개변수 타입 제한이 extends일 경우 - Bunny 이거나 후손만 사용 가능
        // wildCardFarm.extendsType(new RabbitFarm<Rabbit>(new Rabbit())); 불가능
        wildCardFarm.extendsType(new RabbitFarm<Bunny>(new Bunny()));
        wildCardFarm.extendsType(new RabbitFarm<DrunkenBunny>(new DrunkenBunny()));

        // 와일드카드의 매개변수 타입 제한이 super일 경우 - Bunny 이거나 상위만 사용 가능
